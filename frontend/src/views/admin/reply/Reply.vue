<template>
  <a-card :breplyed="false" class="card-area" :bordered="false">
    ...
    <reply-view
      @close="handlereplyViewClose"
      :replyShow="replyView.visiable"
      :replyData="replyView.data">
    </reply-view>
  </a-card>
</template>
...
export default {
  name: 'reply',
  components: {RangeDate, ReplyView},
  data () {
    return {
      replyView: {
        visiable: false,
  ...
  methods: {
    view (row) {
      this.replyView.data = row
      this.replyView.visiable = true
    },
    handlereplyViewClose () {
      this.replyView.visiable = false
    },
    ...
    search () {
      ...
      let sortField, sortreply
      // 获取当前列的排序和列的过滤规则
      if (sortedInfo) {
        sortField = sortedInfo.field
        sortreply = sortedInfo.reply
      }
      this.fetch({
        sortField: sortField,
        sortreply: sortreply,
