            ...
            <a-col :md="6" :sm="24">
              <a-form-item
                ...
                <a-select
                  ...
                  style="width: 100%">
                  <a-select-option key="1">待付款</a-select-option>
                  <a-select-option key="2">待收货</a-select-option>
                  <a-select-option key="3">已完成</a-select-option>
                  <a-select-option key="4">已取消</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
          </div>
    ...
    <div>
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          <a-icon type="inbox" @click="view(record)" title="查 看"></a-icon>
        </template>
      </a-table>
    </div>
    <evaluation-view
...
export default {
  name: 'evaluation',
  components: {RangeDate, EvaluationView},
  data () {
    return {
      evaluationView: {
        visiable: false,
    ...
    columns () {
      return [{
        ...
        customRender: (text, record, index) => {
          if (!record.productImages) return <a-avatar shape="square" icon="user" />
          return <a-popover>
            <template slot="content">
              <a-avatar shape="square" size={132} icon="user" src={ 'http://127.0.0.1:9527/imagesWeb/' + record.productImages.split(',')[0] } />
            </template>
            <a-avatar shape="square" icon="user" src={ 'http://127.0.0.1:9527/imagesWeb/' + record.productImages.split(',')[0] } />
          </a-popover>
        }
      }, {
        title: '评价商品',
  ...
  methods: {
    ...
    fetch (params = {}) {
      ...
      if (params.evaluationStatus === undefined) {
        delete params.evaluationStatus
      }
