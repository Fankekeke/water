<template>
  <a-modal v-model="show" title="修改商品信息" @cancel="onClose" :width="800">
...
export default {
  ...
  methods: {
    ...
    setFormValues ({...product}) {
      ...
      this.tags = []
      this.fileList = []
      let fields = ['productName', 'content', 'productType', 'price', 'stock', 'shopCode', 'standard', 'tagIds']
      let obj = {}
      Object.keys(product).forEach((key) => {
        ...
        if (key === 'productType') {
          product['productType'] = product['productType'].toString()
        }
        if (key === 'tagIds' && product['tagIds'] !== null && product['tagIds'] !== '') {
          this.tags = product['tagIds'].split(',')
        }
